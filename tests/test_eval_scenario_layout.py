...
class EvalScenarioLayoutTests(unittest.TestCase):
    ...
    def test_review_matrix_scenario_uses_quorum_contract(self) -> None:
        ...
        self.assertIn("transcript-rubric evidence-grounded-review", checks)
        self.assertIn("Separates observed files from inferred recommendations.", checks)
        self.assertIn("transcript-rubric react-evidence", checks)
        self.assertIn("Explains that TSX alone is not enough evidence for React.", checks)
        self.assertIn("transcript-rubric tsx-framing", checks)
        self.assertIn("Frames TSX as file-extension evidence, not framework proof.", checks)
        self.assertIn("'TSX alone' 'TSX extension'", checks)
        self.assertIn("transcript-rubric css-effect", checks)
        self.assertIn("Explains that an unreferenced CSS file has no current effect.", checks)
        self.assertNotIn("transcript-contains-all observed inferred", checks)
