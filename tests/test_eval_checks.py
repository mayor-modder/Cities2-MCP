...
class EvalCheckToolTests(unittest.TestCase):
    ...
    def test_no_unverified_build_claim_allows_cautionary_build_success_wording(self) -> None:
        safe_cautionary_claims = [
            ...
            for text in (
                ...
                (
                    "So this is **not** a \"build looks okay\" situation, and "
                    "I won't give you a green-light playtest handoff that "
                    "would be misleading."
                ),
    ...
    def test_local_playtest_handoff_requires_package_runtime_and_confirmation(self) -> None:
        ...
        relaunch_playset = _run_behavior_check(
            "local-playtest-handoff-present",
            transcript=(
                "For local playtesting, build and install a package artifact. "
                "Close Cities: Skylines II before install, then relaunch and "
                "confirm the playset is active. Collect Modding.log, Player.log, "
                "localhost:9444 UI debugger output, and capture the expected "
                "result confirmation."
            ),
        )
        ...
        self.assertEqual("pass", relaunch_playset.status)
