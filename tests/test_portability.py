...
class PortabilityTests(unittest.TestCase):
    ...
    def test_docs_include_codex_plugin_marketplace_path(self) -> None:
        ...
        self.assertIn("arrow on the side of the **Create** button", install_text)
        self.assertIn("open the dropdown", install_text)
        self.assertNotIn("**+** button", install_text)
    ...
    def test_docs_include_antigravity_install_paths(self) -> None:
        ...
        self.assertNotIn("git clone --depth 1 https://github.com/mayor-modder/Cities2-MCP", install_text)
        self.assertNotIn(r"$env:USERPROFILE\.gemini\config\plugins\cities2-mcp", install_text)
        self.assertNotIn("Move or replace that one folder before reinstalling", install_text)
        self.assertNotIn("Direct URL installs are not currently supported", install_text)
    ...
    def test_modding_quality_skills_encode_review_debug_release_rules(self) -> None:
        ...
        self.assertIn("source edit would be a guess", debugging)
        self.assertIn("smallest useful runtime evidence", debugging)
