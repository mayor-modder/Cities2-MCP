...
An override must use clear, informed override language after you restate the missing playtest risk. A casual request such as "release it now" or "it is tiny" is not enough. Without local playtesting or that override, do not create a distribution package, final upload text, or any handoff that calls the mod ready for public upload.
